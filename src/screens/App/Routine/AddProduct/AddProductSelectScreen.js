...
const AddProductSelectScreen = () => {
    const navigation = useNavigation();
    const route = useRoute();
    const insets = useSafeAreaInsets();
    const { products, searchLocalProducts, routineProducts, userData } = useData();
    ...
    // Get all available products (filtered)
    const availableProducts = useMemo(() => {
        const allProducts = Object.values(products);
        
        // Get existing product IDs in user's routine for this routine type
        const existingProductIds = new Set(
            routineProducts
                ?.filter(routineProduct => routineProduct.routineInfo?.routineType === routineType)
                ?.map(routineProduct => routineProduct.productInfo?.id)
                ?.filter(Boolean) || []
        );
        
        // Get disliked product IDs
        const dislikedProductIds = new Set(userData?.routine?.dislikedProducts || []);
        
        // Filter out routine products and disliked products
        const filtered = allProducts.filter(product => 
            !existingProductIds.has(product.id) && 
            !dislikedProductIds.has(product.id)
        );
        
        // Sort by safety score descending
        return filtered.sort((a, b) => (b.safetyScore || 0) - (a.safetyScore || 0));
    }, [products, routineProducts, routineType, userData?.routine?.dislikedProducts]);

    // All search results (not limited) - stored separately
    ...
    useEffect(() => {
        ...
        setCurrentPage(1);
        setHasReachedEnd(false); // Reset end state when products change
    ...
    const handleProductSelect = (product) => {
        navigation.navigate('AddProductEdit', {
            productId: product.id,
            routineType,
            mode: 'add',
            shouldDoubleExit:true,
        });
    };
...
export default AddProductSelectScreen;